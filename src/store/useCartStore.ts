...
export const useCartStore = create<CartState>((set) => ({
    ...
    addProduct: (product: IProduct) => set((state: { products: IProduct[]; }) => {
    ...
    removeProduct: (productId: number) => set((state: { products: IProduct[]; }) => ({
      products: state.products.filter((product: IProduct) => product.id !== productId)
    })),
  

    updateProductCount: (productId: number, count: number = 0) => set((state: { products: IProduct[] }) => ({
        products: state.products.map(p =>
