...
const CardContainer: React.FC<IProps> = ({ item }) => {
    ...
    useEffect(() => {
        if ( data.discount )
            setData({ ...data, priceDiscounted: calculateDiscountedPrice(data.price, data.discount) })
    }, [])

    return <Card item = { data } toggleLike = { toggleLike } changeGrade = { changeGrade } addCart = { addCart } />
}
